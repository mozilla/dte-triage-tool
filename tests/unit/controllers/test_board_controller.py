...
class TestBoardController:
    def test_update_status_map_clears_then_sets(self, session_state, controller):
        ...
        assert session_state.get_status_map() == {1: ("Status Untriaged", "Status Suitable")}
    ...
    def test_normalize_and_save_data_builds_board_and_saves(self, session_state, controller):
        """
        The controller must:
        ...
        for priority_id, status_code in status_priority:
            ...
            assert test_case['fields'] == [f"{controller.priority_translation[priority_id]}"]
            # Ensure colors applied
            assert test_case['color'] == Util.priority_color(priority_id)
        # Saved to state
        assert session_state.get_initial_board() == board

    def test_normalize_and_save_data_handles_empty_cases(self, session_state, controller):
